...
methods{
    ...
    borrowPerSecondInterestRateBase() returns (uint256) envfree;
...
/*
    ...
    @Formula:
        utilization(t) = 0 =>  getBorrowRate(t) = borrowPerSecondInterestRateBase() 
    @Notes:
...
rule utilization_zero(){
    ...
    assert getUtilization(e) == 0 => borrowRate == borrowPerSecondInterestRateBase() ;
}
...
rule presentValue_G_zero(int104 presentValue){
    ...
    assert presentValue_ == to_int256(presentValue) => 
    ( presentValue > 0 <=> principalValue > 0);
}
